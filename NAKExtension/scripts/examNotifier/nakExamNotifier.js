	...
	await fetch(
		...
		.catch((e) => {
			if (e.message === "ExamFetchError") {
				...
				console.log(
					"Failed to check exam grades. Please check your extension permissions"
				);
		});
...
const pdfItemsToGradesObjectMapper = (strings) => {
	...
	const gradeItems = strings.slice(
		pdfItemsExamArrayOffset,
		pdfItemExamArrayEnd
	);
	return gradeItems;
};
...
const checkNakExams = async () => {
	...
	const { nakExamGrades: oldExamGradesString } = await getData({
		nakExamGrades: "",
	});
	...
	if (
		newExamGradesString &&
		!areExamStringsEqual(oldExamGradesString, newExamGradesString)
	) {
		chrome.notifications.create(
			"https://cis.nordakademie.de/studium/pruefungen/pruefungsergebnisse/",
			{
				...
				priority: 1,
				iconUrl: "/NAKExtension-128x128.png",
